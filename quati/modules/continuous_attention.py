...
if __name__ == "__main__":
    from quati.modules.continuous_encoders import LSTMEncoder
    torch.manual_seed(1)
    ...
    # query vectors
    q = torch.randn(batch_size, hidden_size)
    # set of query vectors
    qs = torch.randn(batch_size, source_len, hidden_size)
    # keys (same shape as qs)
    ks = torch.randn(batch_size, source_len, hidden_size)
    # value (same shape as qs)
    vs = torch.randn(batch_size, source_len, hidden_size)

    # self attention
    scorer = LSTMEncoder(hidden_size, hidden_size)
    attn = ContinuousAttention(
        scorer,
        dropout=0.,
        nb_waves=10,
        use_power_basis=True,
        use_wave_basis=True,
    ...
    print(out.shape, probs.shape, ks.shape)
