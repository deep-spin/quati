...
class DiscreteAttentionEncoder(ContinuousEncoder):
    ...
    def __init__(self, vector_size, hidden_size, pool=None, supp_type='pred'):
        from quati.modules.scorer import (SelfAdditiveScorer, ConvScorer)
        super().__init__()
        # alternatively, one can use a more powerful scorer
        # self.scorer = ConvScorer(vector_size, hidden_size)
    ...
    def forward(self, query, keys, mask=None):
        ...
        L = keys.size(1)
        positions = torch.linspace(0, 1, L, device=keys.device)

        # E_p_attn[positions]
        mu = p_attn.matmul(positions).squeeze(1)

        # get sigma_sq according to the supp_type strategy (hardcoded)
        if self.supp_type == 'const':
            L = keys.size(1)
            p = 10.
            sigma_sq = torch.ones_like(mu) * (2. / 3) * (p / L) ** 3

        elif self.supp_type == 'tp':
            # var[positions] according to supp of TP
            sigma_sq = p_attn.matmul(positions ** 2).squeeze(1) - mu ** 2
            sigma_sq = (2. / 3) * ((5. * sigma_sq) ** (3. / 2))
