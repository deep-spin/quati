...
from quati.dataset import corpora
from quati import constants
from quati import models
from quati import optimizer
from quati import scheduler
from quati.dataset.vectors import available_embeddings
from quati.modules.attention import available_max_activations
...
def preprocess_opts(parser):
    ...
    group.add_argument('--f1-pos-label',
                       type=str,
                       default=None,
                       help='Name of the positive label according to tagset.')
    group.add_argument('--f1-average',
                       type=str,
                       choices=['binary', 'micro', 'macro', 'weighted',
                                'samples'],
                       default='macro',
                       help='F1 average type. Check `sklearn.metrics` for a '
                            'complete overview of each choice.')
...
def model_opts(parser):
    ...
    group.add_argument('--embeddings-dropout',
                       type=float,
                       default=0.4,
    ...
    group.add_argument('--attn-type',
                       ...
                       help='The type of attention: regular or multihead.')
    group.add_argument('--attn-domain',
                       type=str,
                       default='discrete',
                       choices=['discrete', 'continuous'],
                       help='Attention domain: discrete or continuous.')
    group.add_argument('--attn-scorer',
                       type=str,
                       default='dot_product',
                       choices=['dot_product', 'general', 'self_add', 'add',
                                'concat', 'mlp', 'lstm', 'conv'],
                       help='Method used to calculate attention similarities.')
