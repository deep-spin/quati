...
from quati.constants import UNK, PAD, START, STOP
...
def load_vectors(path, format, binary=True):
    vectors = None
    # load the word embeddings only if a correct format is provided
    if format is not None:
        assert format in available_embeddings.keys()
        logger.info('Loading {} word embeddings from: {}'.format(format, path))
        emb_cls = available_embeddings[format]
        vectors = emb_cls(path, binary=binary)
    return vectors
